...
describe Action, "Validations" do
  ...
  describe ":kind validity checks" do
    context "when using valid :kind field" do
      specify { Factory.build(:action).should be_valid }
      specify { Factory.build(:action, :kind => 'meeting').should be_valid }
      specify { Factory.build(:action, :kind => 'training').should be_valid }
      specify { Factory.build(:action, :kind => 'event').should be_valid }
      specify { Factory.build(:action, :kind => 'non-county').should be_valid }
    end
