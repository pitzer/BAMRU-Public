  ...
  describe ":kind validity checks" do
    context "when using valid :kind field" do
      it "should be valid" do
        ...
        Factory.build(:event, :kind => 'meeting').should be_valid
        Factory.build(:event, :kind => 'training').should be_valid
        Factory.build(:event, :kind => 'operation').should be_valid
        Factory.build(:event, :kind => 'other').should be_valid
      end
    end
  ...
  describe ":check_dates validity checks" do
    ...
    context "when start and finish are the same" do
      it "should be valid" do
        ...
        Factory.build(:event, valid_dates).should be_valid
      end
