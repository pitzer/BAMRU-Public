...
(Event Logging)
- add an event-log process
- add logging statements to application code

(Monitoring and Alerting)
- add a monitoring process
- generate/send emails

(Process Dashboard)
- show the status of support processes
- allow for process restarts

(Log-File Viewer)
- show log files
- use Faye for live file updates

(Deployment)
- add dual-deploy script
- send secure data via SSH - not stored in repository
- Shared symlink for the database, admin userid/password
- Setup to deployment systems and dns (bamru.info, backup.bamru.info)

(Docco)
- create wiki pages
- add relevant info to BAMRU wiki (account pwds, DNS info, etc.)
...
- Add view tests
- Manually input 2003-2005 calendar events
